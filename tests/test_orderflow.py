...
def test_compute_order_flow_snapshot_detects_velocity_and_delta() -> None:
    ...
    for index in range(40):
        trades.append(
            TradeTick(
                symbol="BTCUSDT",
                venue=Venue.BINANCE,
                timestamp=now - timedelta(seconds=4.8) + timedelta(milliseconds=index * 120),
                price=70600 + index,
                size=0.35,
                side=Side.BUY,
            )
    ...
    assert snapshot.tick_qty_per_5s == 40
...
def test_compute_order_flow_snapshot_ignores_sparse_baseline() -> None:
    ...
    for index in range(25):
        trades.append(
            TradeTick(
                symbol="ALTUSDT",
                venue=Venue.BINANCE,
                timestamp=now - timedelta(seconds=4.8) + timedelta(milliseconds=index * 180),
    ...
    assert snapshot.tick_qty_per_5s == 25

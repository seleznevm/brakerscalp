...
def build_api(
    ...
    async def apply_strategy(
        ...
        pre_alert_confidence_threshold: float = Query(default=75.0, ge=0.0, le=100.0),
        volume_z_threshold: float = Query(ge=0.0, le=20.0),
        watchlist_volume_z_threshold: float = Query(ge=0.0, le=20.0),
        pre_alert_volume_z_threshold: float = Query(default=0.0, ge=0.0, le=20.0),
        min_touches: int = Query(ge=1, le=10),
        squeeze_threshold: float = Query(ge=0.0, le=1.5),
        pre_alert_squeeze_threshold: float = Query(default=0.60, ge=0.0, le=1.5),
        dist_to_level_atr: float = Query(ge=0.0, le=5.0),
        pre_alert_distance_atr_min: float = Query(default=0.2, ge=0.0, le=5.0),
        pre_alert_distance_atr_max: float = Query(default=1.5, ge=0.0, le=5.0),
        ...
        config = StrategyRuntimeConfig(
            ...
            pre_alert_confidence_threshold=pre_alert_confidence_threshold,
            volume_z_threshold=volume_z_threshold,
            watchlist_volume_z_threshold=watchlist_volume_z_threshold,
            pre_alert_volume_z_threshold=pre_alert_volume_z_threshold,
            min_touches=min_touches,
            squeeze_threshold=squeeze_threshold,
            pre_alert_squeeze_threshold=pre_alert_squeeze_threshold,
            dist_to_level_atr=dist_to_level_atr,
            pre_alert_distance_atr_min=pre_alert_distance_atr_min,
            pre_alert_distance_atr_max=pre_alert_distance_atr_max,
...
def _sort_screener_rows(rows: list, *, sort_by: str, sort_dir: str) -> list:
    ...
    status_order = {"actionable": 0, "watchlist": 1, "pre_alert": 2, "arming": 3, "monitor": 4, "cold": 5, "stale": 6, "insufficient": 7}
...
STRATEGY_FIELD_HELP = {
    ...
    "pre_alert_confidence_threshold": "Confidence threshold for an early pre-alert near the level before the breakout starts.",
    ...
    "pre_alert_volume_z_threshold": "Minimum volume z-score for a pre-alert. Usually this can stay near zero because no spike is required yet.",
    ...
    "pre_alert_squeeze_threshold": "Minimum squeeze score required for a pre-alert before the breakout is active.",
    "dist_to_level_atr": "How close price must stay to the level, in ATR units, to qualify as a watch setup.",
    "pre_alert_distance_atr_min": "Minimum ATR distance to the level for a pre-alert. Lower values should already be watchlist or actionable.",
    "pre_alert_distance_atr_max": "Maximum ATR distance to the level for a pre-alert. Farther than this is still too early.",
...
def _runtime_settings_form(
    ...
    *,
    ...
    strategy_fields = ''.join([
        ...
        _strategy_input('pre_alert_confidence_threshold', 'Pre-alert confidence', f'{strategy_config.pre_alert_confidence_threshold:.1f}', tooltip=STRATEGY_FIELD_HELP['pre_alert_confidence_threshold'], min_value='0', max_value='100', step='0.1'),
        ...
        _strategy_input('pre_alert_volume_z_threshold', 'pre_alert_volume_z_threshold', f'{strategy_config.pre_alert_volume_z_threshold:.2f}', tooltip=STRATEGY_FIELD_HELP['pre_alert_volume_z_threshold'], min_value='0', max_value='20', step='0.05'),
        ...
        _strategy_input('pre_alert_squeeze_threshold', 'pre_alert_squeeze_threshold', f'{strategy_config.pre_alert_squeeze_threshold:.2f}', tooltip=STRATEGY_FIELD_HELP['pre_alert_squeeze_threshold'], min_value='0', max_value='1.5', step='0.01'),
        ...
        _strategy_input('pre_alert_distance_atr_min', 'pre_alert_distance_atr_min', f'{strategy_config.pre_alert_distance_atr_min:.2f}', tooltip=STRATEGY_FIELD_HELP['pre_alert_distance_atr_min'], min_value='0', max_value='5', step='0.01'),
        _strategy_input('pre_alert_distance_atr_max', 'pre_alert_distance_atr_max', f'{strategy_config.pre_alert_distance_atr_max:.2f}', tooltip=STRATEGY_FIELD_HELP['pre_alert_distance_atr_max'], min_value='0', max_value='5', step='0.01'),
